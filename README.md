...
When finding the global minimum of f(**Ay**) one needs to impose constraints on the problem to avoid search over infinite domain. We propose two different constraints — each for two different domains (i.e., X ∈ R^D and X ∈ [−1, 1]^D ) — resulting in two different frameworks. The first framework named REGO (Random Embeddings for Global Optimization) defines bound constraints in the new variables, i.e., **y** ∈ Y = [−δ, δ]^d for some user-defined δ. The second one named X-REGO imposes the linear constraints **Ay** ∈ X . So, instead of solving high-dimensional (P), we solve
