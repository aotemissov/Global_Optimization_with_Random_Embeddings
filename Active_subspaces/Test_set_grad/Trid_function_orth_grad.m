% ------------------------------------------------------------- %
% Trid_function_orth_grad(xx,Q,D,dim,bounds)
%
...
% ------------------------------------------------------------- %
function dy_val = Trid_function_orth_grad(xx,Q,D,dim,bounds)
...
dsum = zeros(dim,1);
dsum(1) = 2*(z{1}-1) - z{2};
dsum(dim) = 2*(z{dim}-1) - z{dim-1};

for i = 2:dim-1
	dsum(i) = 2*(z{i}-1) - z{i+1} - z{i-1};
end
